#include "RadiationTests.h"
void InitSuOlsen(GridData *gridData, FixedData const &fixedData, int nx)
{
...
void InitMultiSuOlsen(GridData *gridData, FixedData const &fixedData, int nx, double kappa_bar,double alpha, double density)
{
...
std::vector<double> MultiSuOlsenPlanckIntegral(GridData *gridData, FixedData const& fixedData)
{
...
void InitEquilibrium(GridData *gridData, FixedData const &fixedData, double E_0)
{
...
void InitTransport(GridData *gridData, FixedData const &fixedData, TimeStep &timeStep,  int nx)
{
...
void InitThinStreaming(GridData *gridData, FixedData const &fixedData, TimeStep &timeStep, double Opacity, int nx)
{

    double dx = 100E-2 / (nx + 1);
    timeStep.Dt05 = dx/ (2 * fixedData.SPEED_OF_LIGHT);
    for(int i = 0; i < nx + 1; i++)
    {
        gridData->CellWallCoord.push_back(i * dx);
        if(i > 0)
    ...
    for(int i = 0; i < nx; i++)
    {
        if(gridData->CellCenteredCoord[i] <= 10E-2)
        {
            gridData->RadEnergyDensity[i] = 0.1;
        }
        else
        {

            gridData->RadEnergyDensity[i] = 1E-11;
        }
        gridData->RadRossAbsorptionOpacity[i] = Opacity;
        gridData->RadPlanckAbsorptionOpacity[i] = 0;
        gridData->RadPlanckEmissionOpacity[i] = 0;
        gridData->Density.push_back(1E4);
    }
}
...
TEST(RadiationTransport, HandlesSources)
{ 
    int nx = 100;
    ...
    InitRadSources(&initGridData, T_0, R, nx);
...
TEST(RadiationTransport, HandlesDiffusion)
{
    //Initialise PETSC 
    ...
    std::vector<GridData> variableData;
    GridData initGridData(nx, true);
    InitRadDiffusion(&initGridData, U, D_0, R, nx);
    ...
    // matrixSolver.cleanUpPETScObjects();
}

TEST(RadiationTransport, DISABLED_OptThinStreamingTest)
{
    ...
    fixedData.RightRadBoundaryCondition = 2;
    fixedData.LeftRadBoundaryCondition = 2;
    IO ioData;

    std::vector<bool>bool_list = {false,false,true,true,false,false,false,false,false,false,false,false,false,false,false,false,false,
                               false,false,false,false,false,false,false,false,false,true,false,false,false,false,false,false,false, true};

    for(unsigned long int i = 0; i < bool_list.size(); i++)
    {
        ioData.dumpBools[ioData.dumpVars[i]] = bool_list[i];
    }
    ioData.CreateOutputFolder = true;
    ...
    GridData initGridData(nx, true);
    // MatrixSolver matrixSolver(nx);
    InitThinStreaming(&initGridData, fixedData, timeData, opacity, nx);
    timeData.Tmax = 2E-9;
    Hydro.updateDxs(&initGridData);
    initGridData.dump(ioData, 0);
    variableData.push_back(initGridData);
    radiationTransport.LeftBoundaryValue = initGridData.RadEnergyDensity[0];
    radiationTransport.RightBoundaryValue = initGridData.RadEnergyDensity[nx - 1];
    ...
    while(true)
    {
        ...
        //Units being tested
        radiationTransport.updateRadiationEnergyDensity(&variableData[lastStepDataIndex], fixedData, timeData);
        radiationTransport.ficksDiffusionCoefficient(gridData, fixedData);
        radiationTransport.ficksDiffusion(gridData, fixedData, timeData);
        // radiationTransport.updateRadiationEnergyDensity(gridData);
    ...
    // radiationTransport.cleanUpPETScObjects();
}


std::vector<double> AnalyticEquilibriumResult(double E_0, double dt, int t_step)
{
...
TEST_P(GrayRadiationTransportEquil, Equilibrium_Test)
{
    int nx = 1;
    ...
    std::vector<GridData> variableData;
    GridData initGridData(nx, true);
    InitEquilibrium(&initGridData, fixedData, E_0);
    radiationTransport.storeOpacityTables(opacityTable);
    radiationTransport.setEnergyGroup(j);
    radiationTransport.planckIntegral(&initGridData, fixedData);
    radiationTransport.updateRadiationEnergyDensity(&initGridData, fixedData, timeData);
    ...
    timeData.Dt05 = 1E-12;
    variableData.push_back(initGridData);
    std::vector<double> analytical_form = AnalyticEquilibriumResult(E_0, timeData.Dt05, ceil(timeData.Tmax/timeData.Dt05) + 1);
    // timeData.dump(ioData);
    // variableData.back().dump(ioData, timeData.Step);
    while(true)
    {
        ...
        // if (variableData[lastStepDataIndex].RadEnergyDensity[0] - analytical_form[timeData.Step] < 1e-30)
        // {
        //     relative_error = 1e-5;
        // }
        // else
        // {
        relative_error = (variableData[lastStepDataIndex].RadEnergyDensity[0] - analytical_form[timeData.Step])/analytical_form[timeData.Step];
        // }
        ASSERT_LT(relative_error,1e-2);
    }
}

void writeOut(std::ofstream &variable, std::vector<double> array, int length)
{
    if (!variable)
    {
...
void loadSuOlsenExpected(std::string path, std::vector<double> &SuOlsenU0_100,std::vector<double> &SuOlsenU0_3300,std::vector<double> &SuOlsenU1_100,
...
TEST(MultiGroupRadiationTransport, SuOlsen)
{
    int nx = 1024;
    Init init;
    TimeStep timeData(nx);
    ...
    std::string localPath = loadPath + "/MGSuOlsenVari";
    std::vector<double> SuOlsenU0_100,SuOlsenU0_3300,SuOlsenU1_100,SuOlsenU1_3300,SuOlsenV_100,SuOlsenV_3300;
    loadSuOlsenExpected(localPath, SuOlsenU0_100, SuOlsenU0_3300, SuOlsenU1_100, SuOlsenU1_3300, SuOlsenV_100, SuOlsenV_3300);

    FluidDynamics Hydro(nx);
    std::vector<GridData> variableData;
    GridData initGridData(nx, true);
    InitMultiSuOlsen(&initGridData, fixedData, nx, kappa_bar, alpha, 1.0);
    ...
    std::vector<double> zeroth = MultiSuOlsenPlanckIntegral(&initGridData,fixedData);
    ...
    while(true)
    {
        ...
        std::vector<double> zeroth = MultiSuOlsenPlanckIntegral(&variableData.at(lastStepDataIndex),fixedData);
        radiationTransport.at(0).setPlanckIntegral(zeroth);
        radiationTransport.at(1).setPlanckIntegral(zeroth);

        radiationTransport.at(0).updateRadiationEnergyDensity(&variableData.at(lastStepDataIndex), fixedData, timeData);
        radiationTransport.at(1).updateRadiationEnergyDensity(&variableData.at(lastStepDataIndex), fixedData, timeData);
        if(timeData.TotalTime < tao_0)
        {
            radiationTransport.at(0).heatRegion(heating, timeData);  
            radiationTransport.at(1).heatRegion(heating, timeData);  
        }
        radiationTransport.at(0).ficksDiffusionCoefficient(&variableData.at(lastStepDataIndex), fixedData);
        radiationTransport.at(0).ficksDiffusion(&variableData.at(lastStepDataIndex), fixedData, timeData);
        radiationTransport.at(1).ficksDiffusionCoefficient(&variableData.at(lastStepDataIndex), fixedData);
        radiationTransport.at(1).ficksDiffusion(&variableData.at(lastStepDataIndex), fixedData, timeData);
...
std::vector<double> AnalyticalMGEquilibriumTest(std::vector<double> planck, double opacity, double time, double speed_of_light)
{
...
TEST(MultiGroupRadiationTransport,EquilibriumTest)
{
    FixedData fixedData;
    fixedData.Nx = 1;
    ...
    InitEquilibrium(&initGridData, fixedData, 1e-30);
    ...
    while(true)
    {
        ...
        if(timeData.Step % 1000 == 0)
        {
            ...
            std::vector<double> expected_value = AnalyticalMGEquilibriumTest(all_planck, 1, timeData.TotalTime, fixedData.SPEED_OF_LIGHT);
            std::vector<double> relative_errors;
            for(int i = 0; i < fixedData.RadNg; i++)
            {
...
void loadSuOlsenExpected(std::string path, std::vector<double> &SuOlsenU_100,std::vector<double> &SuOlsenU_3300, std::vector<double> &SuOlsenV_100,std::vector<double> &SuOlsenV_3300)
{
...
TEST(GrayRadiationTransport, SuOlsen)
{
    int nx = 1024;
    Init init;
    TimeStep timeData(nx);
    ...
    std::string loadPath = getenv("TEST_PATH");
    std::string localPath = loadPath + "/MGSuOlsenGray";
    std::vector<double> SuOlsenU_100,SuOlsenU_3163,SuOlsenV_100,SuOlsenV_3163;
    loadSuOlsenExpected(localPath, SuOlsenU_100, SuOlsenU_3163, SuOlsenV_100, SuOlsenV_3163);

    FluidDynamics Hydro(nx);
    std::vector<GridData> variableData;
    GridData initGridData(nx, true);
    InitMultiSuOlsen(&initGridData, fixedData, nx, kappa_bar, alpha, 1.0);
    // InitSuOlsen(&initGridData, fixedData, nx);
    // MatrixSolver matrixSolver0(nx);
    // MatrixSolver matrixSolver1(nx);
    // matrixSolver0.createPETScObjects();
    // matrixSolver1.createPETScObjects();
    ...
    std::vector<double> heating;
    heating.resize(fixedData.Nx);
    double heating_temperature = 1e4;
    for(int i = 0; i < fixedData.Nx; i++)
    {
        if(initGridData.CellCenteredCoord[i] < x0)
        {
...
INSTANTIATE_TEST_SUITE_P(
    GrayRadiationTransportEquili,
    GrayRadiationTransportEquil,
    ::testing::Values(
        std::make_tuple(1E7),
        std::make_tuple(1E11)));