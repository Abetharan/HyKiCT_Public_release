...
int main(int argc, char* argv[])
{
    ...
    if(!switchData.LeakSource)
    {
        constantData.leakArea1 = 0.0;
        constantData.leakArea2 = 0.0;
    }
    ...
    {   
        ...
        if(!switchData.IsothermalModeON)
        {
            ...
            else
            {
                ...
                // Hydro.updateTemperatureEOperatorSplit(&variableData.at(lastStepDataIndex - 1), &variableData.at(lastStepDataIndex), timeData);
                // double tmpDt = timeData.Dt05;
                // timeData.Dt05 *=0.5;
                ...
                // variableData.at(lastStepDataIndex).HeatKappaE = variableData.at(lastStepDataIndex -1 ).HeatKappaE; // Carry over Multipliers
                ...
                variableData.at(lastStepDataIndex).TemperatureI = variableData.at(lastStepDataIndex -1 ).TemperatureI; // Carry over Multipliers
                ...
                else
                {
                    ...
                    plasmaParams.calculateLeeMoreCoulombLogs(&variableData.at(lastStepDataIndex), constantData);
                }
                internalSources.calculateKappa(&variableData.at(lastStepDataIndex), constantData);
                // if(switchData.LeeMore)
                // {
                //     internalSources.calculateLeeMoreKappaECorr(&variableData.at(lastStepDataIndex), constantData);
                // }
                if(switchData.HeatConductionOn) 
                {
                    if(!switchData.SNBHeatFlow)
                    {
                        internalSources.calculateImplicitKappaE(&variableData.at(lastStepDataIndex), constantData);
                        if(switchData.Couple)
                        {
                            Hydro.updateTemperatureImplicitHeatConduction(variableData.at(lastStepDataIndex).TemperatureE, variableData.at(lastStepDataIndex).HeatKappaE, variableData.at(lastStepDataIndex).SpecificHeatE,
                                                                        variableData.at(lastStepDataIndex).Mass, variableData.at(lastStepDataIndex).Dx_k, timeData.Dt05, &variableData.at(lastStepDataIndex).HeatFlowMultiplier,
                                                                        switchData.CoupleMulti);
                        }
                        else
                        {
                            Hydro.updateTemperatureImplicitHeatConduction(variableData.at(lastStepDataIndex).TemperatureE, variableData.at(lastStepDataIndex).HeatKappaE, variableData.at(lastStepDataIndex).SpecificHeatE,
                        ...
                        //Purely diagonistic output is not used anywhere besides for dumping
                        internalSources.diagonsticHeatFlowE(&variableData.at(lastStepDataIndex), constantData, switchData);                    
                    ...
                    else
                    {
                        ...
                        internalSources.thermalConducE(&variableData.at(lastStepDataIndex));
                        Hydro.updateTemperatureEOperatorSplit(&variableData.at(lastStepDataIndex), timeData, &variableData.at(lastStepDataIndex).HeatConductionE);
                    // Hydro.updateTemperatureE(&variableData.at(lastStepDataIndex - 1), &variableData.at(lastStepDataIndex), timeData, switchData.AdibaticModeON);
                    }
                }
            }
            ...
            {   
                // std::fill(variableData.at(lastStepDataIndex - 1).HeatConductionI.begin(), variableData.at(lastStepDataIndex - 1).HeatConductionI.end(), 0);
                ...
                // internalSources.calculateKappa(&variableData.at(lastStepDataIndex), constantData);
                // internalSources.calculateImplicitKappaI(&variableData.at(lastStepDataIndex), constantData);
                // variableData.at(lastStepDataIndex).SpecificHeatI = variableData.at(lastStepDataIndex - 1).SpecificHeatI;
                // Hydro.updateTemperatureImplicitHeatConduction(variableData.at(lastStepDataIndex).TemperatureI, variableData.at(lastStepDataIndex).HeatKappaI, variableData.at(lastStepDataIndex).SpecificHeatI,
                                                            // variableData.at(lastStepDataIndex).Mass, variableData.at(lastStepDataIndex).Dx_k, timeData.Dt05);
            }
        }
        else
    ...
    else
    {
        ...
        {
            plasmaParams.calculateCollisionFrequencyEI(&variableData.at(lastStepDataIndex), constantData, false);
        ...
        if(switchData.RadTransportOn)
        {
            ...
            // radiationTransport.at(0).calculateRadPressure(&variableData.at(lastStepDataIndex), constantData);
        }
